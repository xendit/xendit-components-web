...
describe("channel component state/province field test", () => {
  ...
  describe("browser autofill via the hidden native select", () => {
    // simulates the browser autofil, writing a value directly onto the native `<select>` and firing its change event
    function autofillNativeSelect(select: HTMLSelectElement, value: string) {
      select.value = value;
      select.dispatchEvent(new Event("change"));
    }

    it("clears the province when the browser clears the select", async () => {
      const sdk = new XenditComponentsTest({
        componentsSdkKey: "test-client-key",
      });
      ...
      expect(channelProperties).toMatchObject({ province_field: "" });
      expect(screen.getAllByRole("button")[1].textContent).toBe(
        "State / Province",
      );
    });

    it("updates the province when the browser fills a province we offer", async () => {
      const sdk = new XenditComponentsTest({
        componentsSdkKey: "test-client-key",
      });
      ...
      autofillNativeSelect(select, "NY");
      await sleep(1); // wait for the dropdown UI to re-render with the new value
      ...
      expect(channelProperties).toMatchObject({ province_field: "NY" });
      expect(screen.getAllByRole("button")[1].textContent).toBe("New York");
    });

    it("keeps the previous province when the browser fills a value we don't offer", async () => {
      const sdk = new XenditComponentsTest({
        componentsSdkKey: "test-client-key",
      });
      ...
      // a province the autofill might write that isn't in our list
      ...
      expect(channelProperties).toMatchObject({ province_field: "CA" });
      expect(screen.getAllByRole("button")[1].textContent).toBe("California");
      expect(select.value).toBe("CA");
    });
  });
});
